...
export default {
    prompt: "There is an integer array nums sorted in ascending order (with distinct values). Prior to being passed to" +
        ...
        "example, [0,1,2,4,5,6,7] might be rotated at pivot index 3 and become [4,5,6,7,0,1,2]. Given the array nums" +
        "after the possible rotation and an integer target, return the index of target if it is in nums, or -1 if it is" +
        "not in nums. You must write an algorithm with O(log n) runtime complexity.",
    tests: [
        {
            test: (f) => run(f, [4, 5, 6, 7, 0, 1, 2], 0),
            expected: 4,
            testCase: "Example 1"
        },
        {
            test: (f) => run(f, [4, 5, 6, 7, 0, 1, 2], 3),
            expected: -1,
            testCase: "Example 2"
        },
        {
